...
from keras.layers import Input, ZeroPadding2D, concatenate, add, multiply, Lambda, Activation
...
def coral_loss(y_true, y_pred):
    return y_pred
def sinnet(weightfile):
    concat_axis = 3

    img_input = Input(shape=(224, 224, 1), name="img_input")
    ...
    dense2 = Dense(2, activation=None, kernel_initializer="normal", name='dense2')(dense1)

    lesion_class = Activation('softmax', name='class')(dense2)

    model_base = Model(img_input, [mask, lesion_class], name='sinnet')

    model_base.load_weights(weightfile)

    return model_base


def sinnet_share(weightfile, lr=1e-7):
    input1 = sinnet(weightfile)
    input2 = sinnet(weightfile)
    ...
    mask1, lesion_class_1 = input1.output
    dense2_model1 = input1.get_layer(name="dense2").output
    dense2_model2 = input2.get_layer(name="dense2_2").output

    coral_out = Lambda(lambda x: CORAL(*x), name="coral")([dense2_model1, dense2_model2])

    model = Model([inp1, inp2], [mask1, lesion_class_1, coral_out], name='sinnet_share')

    losses = {
        "class": 'categorical_crossentropy',
        "mask": dice_coef_loss,
        "coral": lambda y_true, y_pred: y_pred
    }
...
def main_testing(dicom_T2_path):
    ...
    x1, y1, x2, y2 = predict(dicom_T2_path)

    # return malignacy and save './media/output/T2_lesion.png'
    prostate_roi_T2_path='./media/output/prostate_roi.png'
    malignancy_T2 = predict_malignancy_T2(prostate_roi_T2_path,dicom_T2_path)
    ...
    if len(results[0]['rois']) > 1:
        ...
        if len(list_index) > 1:
            ...
            for i in range(len(list_index) - 1):
                rois = np.concatenate((rois, results[0]['rois'][list_index[i + 1]].reshape(1, -1)), axis=0).reshape(
                    -1, 4)
                masks = np.concatenate((masks, results[0]['masks'][:, :, list_index[i + 1]].reshape(512, 512, 1)),
                                       axis=2).reshape(512, 512, -1)
                class_ids = np.concatenate((class_ids, results[0]['class_ids'][list_index[i + 1]].reshape(1, -1)),
                                           axis=0).reshape(-1, 1)
    ...
    if len(r['rois']) != 0:
        ...
        for object in range(len(r['rois'])):
            ...
            if class_id==3:
                ...
                cv2.drawContours(image_with_contour, [np.array(prostate_contour)], -1, color, thickness=2)
                ...
                # Image.open()
                # Image.fromarray(mask, mode='L').resize((w,h)).save(
                #     save_path + str(class_names[class_id]) + '_' + str(roi[0]) + '_' + str(roi[1]) + '_' + str(
                #         roi[2]) + '_' + str(
                #         roi[3]) + '.png')
...
def bval_transform(bval_path,T2_path,x1_T2,y1_T2,x2_T2,y2_T2):
    ...
    try:
        ...
        draw = ImageDraw.Draw(im)
        draw.rectangle((x1_raw_bval, y1_raw_bval, x2_raw_bval, y2_raw_bval), outline=(255, 255, 0), width=3)
...
def predict_malignancy_T2(prostate_roi,raw_image_path):
    ...
    if os.path.exists(prostate_roi):
        ...
        if np.size(contours) != 0:
            ...
            image_with_contour = cv2.drawContours(raw_image_RGB,[contour],-1, color=(0, 255, 255),lineType = cv2.LINE_AA,thickness=-1)
            #method 2
            #image_with_contour = cv2.drawContours(raw_image_RGB, [contours[0]], -1, color=(0, 0, 255),thickness=-1)
...
# if __name__ == '__main__':
    # requirements:
    # make sure that the users are only allowed to submit the files with required format, otherwise fail to submit
    # make sure the only T2 image is allowed,which is submitted under './media/upload/T2', ADC, BVAL is not allowed
    # make sure the files under './media/upload/T2' are removed before new submission, which means only one latest file is allowed in respectice folder for each testing
    # instruct the user to submit T2 in png,jpg, or jpeg


    # remove exsiting files under './media/convert_img/T2,ADC,BVAL'
    # the uploaded files in './media/upload/T2,ADC,BVAL' will be transformed to png under './media/convert_img/T2,ADC,BVAL'
    for file in os.listdir('./media/convert_img/T2/'):
        os.remove(os.path.join('./media/convert_img/T2/',file))

    for file in os.listdir('./media/convert_img/ADC/'):
        os.remove(os.path.join('./media/convert_img/ADC/',file))
    
    for file in os.listdir('./media/convert_img/BVAL/'):
        os.remove(os.path.join('./media/convert_img/BVAL/',file))

    # for file in os.listdir('./media/upload/T2'):
    #     os.remove(os.path.join('./media/upload/T2', file))
    #
    # for file in os.listdir('./media/upload/ADC'):
    #     os.remove(os.path.join('./media/upload/ADC', file))
    #
    # for file in os.listdir('./media/upload/BVAL'):
    #     os.remove(os.path.join('./media/upload/BVAL', file))

    try:
        T2_dicom = os.listdir('./media/upload/T2/')
        if len(T2_dicom)==1:
            if os.path.isfile(os.path.join('./media/upload/T2/',T2_dicom[0])):  
                if T2_dicom[0].endswith("png") or T2_dicom[0].endswith("jpg") or T2_dicom[0].endswith("jpeg"):
                    T2_dicom_path = os.path.join('./media/upload/T2/', T2_dicom[0])
                    malignancy=main_testing(T2_dicom_path)
        ...
        raise RuntimeError('fail to test')
